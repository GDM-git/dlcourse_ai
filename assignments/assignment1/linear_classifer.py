...
def cross_entropy_loss(probs, target_index):
    ...
    if probs.ndim == 1:
        ...
        # loss = np.average(-np.log(probs[np.arange(batch_size), use_index]))
        # loss = np.sum(-np.log(probs[np.arange(batch_size), use_index])) / batch_size
    return loss
...
def softmax_with_cross_entropy(predictions, target_index):
    ...
    if predictions.ndim == 1:
        ...
        batch_size = np.array(target_index).size
        dprediction[np.array(range(batch_size)), target_index] -= 1
...
class LinearSoftmaxClassifier():
    ...
    def fit(self, X, y, batch_size=100, learning_rate=1e-7, reg=1e-5,
        ...
        for epoch in range(epochs):
            ...
            # and regularization!
            loss_epoch, num_batches = 0, 0
            for batch in batches_indices:
                num_batches += 1
                lin_loss, lin_grad = linear_softmax(X[batch], self.W, y[batch])
                reg_loss, reg_grad = l2_regularization(self.W, reg)
                loss_batch = lin_loss + reg_loss
                loss_epoch += loss_batch
                loss_history.append(loss_batch)
                self.W -= learning_rate * (lin_grad + reg_grad)
            if num_batches != 0:
